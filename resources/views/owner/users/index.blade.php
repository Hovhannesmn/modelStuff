...
@section('content')
    ...
    <li class="filter-type">
      <a href="/owner/profile/users/create"><i class="fa fa-user-plus"></i>&nbsp;&nbsp;&nbsp;{{ Languages::trans('admin.users.create_user') }}</a>
    </li>
    <li class="filter-type">
      {{ Languages::trans('general.labels.show') }}:
      <a {!! Request::is('owner/users') ? 'class="active"' : '' !!} href="{{ route('admin.users.index') }}">{{ Languages::trans('general.buttons.all') }} ({{ $users->count() }})</a>
{{--      <a {!! Request::is('owner/users/admin') ? 'class="active"' : '' !!} href="{{ route('admin.users.show', 'admin') }}">{{ Languages::trans('admin.users.administrators') }}</a>--}}
{{--      <a {!! Request::is('owner/users/model') ? 'class="active"' : '' !!} href="{{ route('admin.users.show', 'model') }}">{{ Languages::trans('admin.users.models') }}</a>--}}
    </li>
	</ul>
          ...
          <tr class="user-row">
            ...
            <td>
              @if( $user->hasRole('model') )
                @if( !$user->profile )
                  <a href="/owner/profile/users/{{ $user->id }}/newprofile" class="text-success"><i class="fa fa-female"></i> +</a>
                @else
                  <a href="/owner/profile/{{ $user->profile->id }}" class="text-muted"><i class="fa fa-female"></i></a>
                  &nbsp;&nbsp;&nbsp;
                  <a href="/owner/profile/{{ $user->profile->id }}/edit" class="text-muted"><i class="fa fa-cog"></i></a>
                @endif
              @endif
            </td>
            <td class="table-action">
              @if(!empty($user->profile->id))
                <a href="/owner/profile/{{ $user->profile->id }}/edit"><i class="fa fa-pencil"></i></a>
              @else
                <a href="/owner/profile/users/{{ $user->id }}/newprofile"><i class="fa fa-pencil"></i></a>
              @endif
    ...
    jQuery(document).ready(function($){
      ...
      var deleteItems = function(checked){
        ...
        if(id.length > 0){
          ...
          $.ajax({
            method: 'DELETE',
            url: '{!! route("owner.profile.users.index") !!}/' + id.join('-'),
      ...
      $('.btn-user-remove').click(function(event){
        ...
        if(confirm("{!! Languages::trans('admin.users.delete_confirm_single') !!} "+ user.find('.user-name').text() +"from your lovehouse?")){
      ...
      $('#deleteall').click(function(event){
        ...
        if(confirm("{!! Languages::trans('admin.users.delete_confirm_multiple') !!} from your lovehouse?")){
